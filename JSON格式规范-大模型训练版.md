# 煤矿采空区数据集 - JSON格式规范（大模型训练版）

## 🎯 设计原则

### 一个矿一个JSON文件

**原因**:
1. ✅ **完整上下文** - 保留采空区各要素之间的关联关系
2. ✅ **业务边界清晰** - 符合"一个煤矿"的自然边界
3. ✅ **便于模型学习** - 模型能学习表与表之间的关系
4. ✅ **易于管理** - 添加新煤矿只需新增文件
5. ✅ **便于采样** - 训练时可按煤矿采样

---

## 📋 JSON结构

### 顶层结构

```json
{
  "mine_info": {...},      // 煤矿基本信息
  "statistics": {...},     // 数据统计
  "data": {...}            // 10个表的数据
}
```

### 完整示例

```json
{
  "mine_info": {
    "mine_id": "HX001",
    "mine_name": "TEST煤矿",
    "survey_date": "2024年",
    "standard": "KAT 22.2-2024",
    "data_version": "1.0.0"
  },
  
  "statistics": {
    "goaf_basic_info": 30,
    "goaf_water_info": 17,
    "goaf_gas_info": 124,
    "fire_info": 0,
    "suspended_roof_info": 10,
    "collapse_info": 0,
    "crack_info": 10,
    "abandoned_shaft_info": 2,
    "seal_wall_info": 25,
    "treatment_info": 4
  },
  
  "data": {
    "goaf_basic_info": [
      {
        "mine_id": "HX001",
        "mine_name": "TEST煤矿",
        "goaf_id": "HX001-G001",
        "goaf_name": "采空1",
        "coal_seam": "3-1",
        "mining_method": "房柱式",
        "goaf_area": 298,
        "area_unit": "km²",
        ...
      }
    ],
    
    "goaf_water_info": [
      {
        "water_id": "HX001-W001",
        "goaf_id": "HX001-G005",
        "water_number": "B3-1",
        "water_type": "采空区积水",
        ...
      }
    ],
    
    "goaf_gas_info": [...],
    "fire_info": [...],
    "suspended_roof_info": [...],
    "collapse_info": [...],
    "crack_info": [...],
    "abandoned_shaft_info": [...],
    "seal_wall_info": [...],
    "treatment_info": [...]
  }
}
```

---

## 📊 字段映射

### 表名映射（中文 → 英文）

| 中文表名 | 英文字段名 | 说明 |
|---------|-----------|------|
| 采空区基本信息 | goaf_basic_info | 采空区基础数据 |
| 采空区积水信息 | goaf_water_info | 积水相关数据 |
| 采空区积气信息 | goaf_gas_info | 气体监测数据 |
| 自燃发火信息 | fire_info | 火区相关数据 |
| 采空区悬顶信息 | suspended_roof_info | 悬顶相关数据 |
| 采空区塌陷信息 | collapse_info | 塌陷相关数据 |
| 地裂缝信息 | crack_info | 裂缝相关数据 |
| 废弃井筒信息 | abandoned_shaft_info | 井筒相关数据 |
| 密闭墙信息 | seal_wall_info | 密闭墙相关数据 |
| 采空区治理信息 | treatment_info | 治理工程数据 |

---

## 🔧 转换工具使用

### 安装依赖

```bash
pip install pandas
```

### 转换单个煤矿

```python
from CSV转JSON工具 import MineDatasetConverter

# 创建转换器
converter = MineDatasetConverter("HX001", data_dir=".")

# 转换为JSON
converter.convert_to_json("HX001-采空区数据集.json")
```

### 批量转换

```python
# 批量转换多个煤矿
mine_codes = ["HX001", "SB001", "WCH001"]
converter = MineDatasetConverter("", data_dir=".")
converter.batch_convert(mine_codes, output_dir="./json_output")
```

---

## 📁 文件组织

### 推荐目录结构

```
dataset/
├── raw/                          # 原始CSV文件
│   ├── HX001-采空区基本信息.csv
│   ├── HX001-采空区积水信息.csv
│   └── ...
├── json/                         # 转换后的JSON文件
│   ├── HX001-采空区数据集.json
│   ├── SB001-采空区数据集.json
│   └── WCH001-采空区数据集.json
├── train/                        # 训练集
│   ├── HX001-采空区数据集.json
│   └── ...
├── val/                          # 验证集
│   └── ...
└── test/                         # 测试集
    └── ...
```

---

## 🎓 大模型训练建议

### 1. 数据划分

```python
# 按煤矿划分训练/验证/测试集
train_mines = ["HX001", "SB001", ...]  # 70%
val_mines = ["WCH001", ...]            # 15%
test_mines = ["XXX001", ...]           # 15%
```

### 2. 数据增强

- ✅ 保持一个煤矿的完整性
- ✅ 可以对单个煤矿内的记录进行采样
- ✅ 可以生成问答对（基于表间关系）

### 3. 训练任务

**任务1: 信息抽取**
```
输入: TEST煤矿的采空区基本信息
输出: 提取采空区数量、煤层、开采方式等
```

**任务2: 关系推理**
```
输入: 采空区G001的基本信息
输出: 推理该采空区的积水情况、气体监测结果等
```

**任务3: 风险评估**
```
输入: 完整的煤矿数据
输出: 评估采空区的风险等级
```

---

## 💡 优势分析

### vs 多个JSON文件

| 维度 | 一个矿一个JSON | 多个表多个JSON |
|------|---------------|---------------|
| **上下文学习** | ⭐⭐⭐⭐⭐ 完整 | ⭐⭐ 碎片化 |
| **关系学习** | ⭐⭐⭐⭐⭐ 自然 | ⭐⭐ 需要拼接 |
| **数据管理** | ⭐⭐⭐⭐⭐ 简单 | ⭐⭐⭐ 复杂 |
| **采样策略** | ⭐⭐⭐⭐⭐ 灵活 | ⭐⭐⭐ 受限 |
| **模型理解** | ⭐⭐⭐⭐⭐ 直观 | ⭐⭐⭐ 抽象 |

---

## 📝 数据质量保证

### 1. 完整性检查

```python
def check_completeness(json_data):
    """检查数据完整性"""
    required_tables = [
        "goaf_basic_info",
        "goaf_water_info",
        "goaf_gas_info",
        ...
    ]
    
    for table in required_tables:
        if table not in json_data["data"]:
            print(f"❌ 缺少表: {table}")
        elif len(json_data["data"][table]) == 0:
            print(f"⚠️ 表为空: {table}")
```

### 2. 关系一致性检查

```python
def check_relationships(json_data):
    """检查外键关系"""
    goaf_ids = {item["goaf_id"] for item in json_data["data"]["goaf_basic_info"]}
    
    for water in json_data["data"]["goaf_water_info"]:
        if water["goaf_id"] and water["goaf_id"] not in goaf_ids:
            print(f"❌ 外键错误: {water['water_id']}")
```

---

## 🎯 最佳实践

### 1. 文件命名

```
{mine_code}-采空区数据集.json

示例:
- HX001-采空区数据集.json
- SB001-采空区数据集.json
- WCH001-采空区数据集.json
```

### 2. 数据组织

- ✅ 一个煤矿一个JSON文件
- ✅ 保持表的顺序一致
- ✅ 使用统一的字段名
- ✅ 空值使用null而不是空字符串

### 3. 版本管理

```json
{
  "mine_info": {
    "data_version": "1.0.0",
    "schema_version": "1.0.0",
    "update_date": "2024-12-26"
  }
}
```

---

## 📚 参考资料

- **Schema定义**: 煤矿采空区普查数据集Schema.json
- **转换工具**: CSV转JSON工具.py
- **标准文档**: KAT 22.2-2024

---

**版本**: 1.0.0  
**更新日期**: 2024-12-26  
**维护单位**: 煤矿采空区普查数据标准化工作组

